    ...
    <div class="stats">
        ...
        <div class="stats-grid">
            <div class="stats-row">
                ...
                <div class="stats-cell">{{ $students->count() }}</div>
                <div class="stats-cell">Total candidatures :</div>
                <div class="stats-cell">{{ $students->sum(function($student) { return $student->applications->count(); }) }}</div>
            </div>
            <div class="stats-row">
                <div class="stats-cell">Candidatures acceptées :</div>
                <div class="stats-cell">{{ $students->flatMap->applications->where('status', 'accepted')->count() }}</div>
                <div class="stats-cell">Candidatures en cours :</div>
                <div class="stats-cell">{{ $students->flatMap->applications->where('status', 'in_progress')->count() }}</div>
            </div>
        </div>
    </div>
        ...
        <div class="student">
            ...
            <div class="student-info">
                ...
                <div class="info-row">
                    ...
                    <div class="info-value">{{ $student->postal_code }}</div>
                </div>
                @endif
                
                <div class="info-row">
                    <div class="info-label">Pays :</div>
                    <div class="info-value">{{ $student->country ?? 'Sénégal' }}</div>
                </div>
                
                <div class="info-row">
                    <div class="info-label">Inscrit le :</div>
                    <div class="info-value">{{ $student->created_at->format('d/m/Y à H:i') }}</div>
                </div>
            </div>
            
            <div class="applications">
                <h4>Candidatures ({{ $student->applications->count() }})</h4>
                @if($student->applications->count() > 0)
                    @foreach($student->applications as $application)
                        <div class="application">
                            <div class="application-school">{{ $application->school->name }}</div>
                            <div style="margin-top: 5px;">
                                ...
                                <span style="margin-left: 10px; font-size: 11px; color: #666;">
                                    Domaine: {{ $application->field_of_study }}
                                </span>
                                <span style="margin-left: 10px; font-size: 11px; color: #666;">
                                    Niveau: {{ $application->diploma_level }}
                                </span>
                            </div>
                            ...
                            @if($application->commission_amount > 0)
                                <div style="margin-top: 5px; font-size: 11px; color: #666;">
                                    Commission: {{ number_format($application->commission_amount, 0, ',', ' ') }} CFA
                                </div>
                            @endif
                        </div>
                    @endforeach
